...
async function main() {
  ...
  if (available) {
    ...
    console.log('⚠️  Pandoc binary not available. Install it to run conversions.\n');
...
`;
  ...
  try {
    ...
    if (styledHtmlResult.success) {
      ...
      console.log('Document length:', styledHtmlResult.output?.length, 'characters\n');
    } else {
    ...
    const shortResult = await md2html('# Quick Test\n\nThis is a **quick** test.');
    ...
    const htmlInput = '<h1>Hello</h1><p>This is <strong>bold</strong> and <em>italic</em> text.</p>';
    ...
    if (directResult.success) {
      ...
      console.log('Has custom styling:', directResult.output?.includes('Arial') ? 'Yes' : 'No');
    }
    ...
    try {
      ...
      console.log('Common input formats:', inputFormats.filter(f =>
        ['markdown', 'html', 'docx', 'latex', 'rst'].includes(f)
      ).join(', '));
      console.log('Common output formats:', outputFormats.filter(f =>
        ['html', 'pdf', 'docx', 'epub', 'latex'].includes(f)
      ).join(', '));
    } catch (error) {
