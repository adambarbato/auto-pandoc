...
export class Pandoc {
  ...
  static async getBinaryPath(): Promise<string> {
    ...
    console.log(
      "Pandoc binary not found. Attempting automatic installation...",
    );
    try {
    ...
    throw new Error(
      "Pandoc binary not found. Please run 'npm run install-pandoc' to install the binary manually.",
    );
  }
   ...
   */
  static async getBinaryInfo(): Promise<PandocBinary> {
    try {
   ...
   */
  private static buildArgs(options: PandocOptions): string[] {
    const args: string[] = [];
